...
- **서버 재설치 시 빠뜨리기 쉽다.** 새로 프로비저닝한 노드는 이 파일이 없는 상태로 시작한다 — [`06-keepalived.sh`](../scripts/03-mysql-ha/06-keepalived.sh)가 기본으로 생성해주지 않으면 mysqld는 정상인데 마스터로 승격이 안 되는 상태가 된다.
