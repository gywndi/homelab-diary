...
BE/CN은 여러 개라 죽어도 나머지가 버텨주지만, FE가 1개뿐이면 그 순간 클러스터 전체가 멎는다. `systemctl restart starrocks-fe`는 짧으면 수 초, 길면 수십 초 걸릴 수 있다 — 운영 중 재시작이 필요하면 이 다운타임을 감안해야 한다. FE를 여러 개로 늘리는 게 정답이지만(위 "설계 결정" 참고), 이 클러스터에서는 의도적으로 안 했다.

### 이미 실행 중인 인스턴스 디렉터리를 `cp -r`로 복제하면 안 된다
CN 인스턴스를 처음 만들 때 "이미 떠 있는 BE 디렉터리를 그대로 복사하면 되지 않을까" 시도했다가, `bin/*.pid`와 `meta`/`storage` 아래의 BE 실행 상태(클러스터 ID, 등록 정보 등)까지 같이 복사돼 "Backend running as process N. Stop it first." 에러와 원인 불명의 등록 충돌을 겪었다. `08-install-cn.sh`가 매번 tarball을 새로 받아 압축을 푸는 것도 이 때문이다 — 실행된 적 없는 깨끗한 디렉터리를 복사하면 이 문제 자체가 생기지 않는다. 기존 인스턴스를 복제해 새 인스턴스를 만드는 방식은 피할 것.

### `tablet_create_timeout_second`를 60으로 올려도 첫 테이블 생성이 타임아웃날 수 있다
CN을 막 띄운 직후(콜드스타트)엔 RGW와의 첫 연결 자체에 시간이 걸려서, 이미 기본값(10초)보다 6배 늘린 60초로도 첫 `CREATE TABLE`이 타임아웃 나는 경우를 봤다. 몇 분 뒤 재시도하면 별다른 설정 변경 없이 통과했다 — CN/FE가 완전히 워밍업된 뒤에 첫 쓰기를 시도하거나, 콜드스타트 직후라면 타임아웃을 더 넉넉히(120초 이상) 잡을 것.
