...
[← 이전: LLM GPU 노드](05-llm-gpu-node.md) · [다음: StarRocks 베어메탈 →](06-2-kvm-starrocks.md)

Kubernetes로 옮기기 어려운 워크로드를 위한 대안. 3노드(chan08/chan09/llm001) 전부에 libvirt와 전용 스토리지 풀, VM 간 실 LAN 통신용 브리지를 갖춰뒀다.

## 목적

컨테이너화가 어려운 워크로드를 위해 KVM 하이퍼바이저 기반을 갖춘다. 특정 OS가 필요하거나 그대로 옮기기 곤란한 경우가 대상이다. VM이 물리 노드를 넘나들며 서로 통신해야 하는 경우(예: 여러 노드에 걸친 클러스터)를 대비해, NAT 전용 기본 네트워크가 아니라 물리 LAN에 직접 붙는 브리지까지 준비해둔다.
...
- **VM 디스크는 XFS(`/mnt/starrocks-be`) 위에.** 원래는 전용 `/data/vms` 파티션을 쓸 계획이었으나, Ceph 도입 과정에서 그 파티션 자체가 재분할돼 없어졌다(아래 "알려진 이슈" 참고). 남은 로컬 XFS 파티션의 여유 공간(노드당 400G 이상)을 그대로 쓴다 — Ceph RBD로 옮기는 방안도 검토했지만([`07-1-ceph-storage.md`](07-1-ceph-storage.md) 설계 결정 참고), VM은 실험적 용도라 설정 단계를 하나 더 늘릴 이유가 약했다.
- **물리 NIC을 브리지로 바꾼다.** libvirt 기본 네트워크(`virbr0`)는 NAT라 그 안의 VM은 물리 LAN의 다른 호스트(다른 노드의 VM 포함)와 직접 통신할 수 없다. 여러 노드에 걸친 VM 클러스터를 만들려면 VM이 실제 LAN IP(`10.5.5.0/24`)를 받아야 하므로, 노드의 유일한 물리 NIC(`enp1s0`)를 브리지(`br0`)로 바꾸고 VM을 거기에 붙인다.
...
- **VM은 cloud-init(NoCloud)으로 완전 자동 프로비저닝한다.** Ubuntu 클라우드 이미지를 그대로 백킹 디스크로 쓰고, `cloud-localds`로 만든 seed ISO에 계정/SSH 키/고정 IP를 심어서 최초 부팅부터 SSH가 바로 되게 한다. 콘솔로 손수 설치할 필요가 없다.
...
# 남은 XFS 파티션 위에 vm-pool이라는 이름의 저장 공간 등록
sudo mkdir -p /mnt/starrocks-be/vm-disks
sudo virsh pool-define-as vm-pool dir --target /mnt/starrocks-be/vm-disks
...
### VM 생성
- 설명: Ubuntu 24.04 VM 1개를 만든다. cloud-init으로 계정/SSH 키/고정 IP까지 자동 설정된다.
- 스크립트: [`03-create-vm.sh`](../scripts/06-kvm/03-create-vm.sh)
```bash
sudo ./03-create-vm.sh starrocks-vm1 10.5.5.52 10.5.5.1
```
핵심 부분(전체는 스크립트 참고):
```bash
...
최초 설계 시점엔 전용 `/data` 파티션 아래 `data-pool`을 쓸 계획이었다. 이후 Ceph를 cephadm으로 재구축하면서 디스크 전체가 재분할돼 `/data`라는 마운트 자체가 없어졌다(`sda1`=Ceph OSD LVM, `sda2`=`/mnt/starrocks-be`로 재편). VM을 실제로 만들려는 시점에야 이 사실을 발견했다 — 스토리지 풀 경로를 남은 XFS 파티션(`/mnt/starrocks-be/vm-disks`)으로 다시 잡았다. 인프라 설계 문서에 "나중에 정한다"고 미뤄둔 값은, 정말 그 시점이 왔을 때 반드시 다시 확인해야 한다는 교훈.
