...
8. StarRocks 분석 엔진 — 1. k8s 구축, 2. 베어메탈 구축
   - Ceph RGW를 오브젝트 스토리지로 쓰는 shared-data 클러스터 + 비교용 shared-nothing 클러스터. k8s(FE/CN) 버전과 KVM VM 위 systemd 베어메탈 버전 둘 다 구성해봤다.
   - k8s 구축 문서 [바로가기](lessons/08-1-starrocks-analytics.md) · 베어메탈 구축 문서 [바로가기](lessons/08-2-starrocks-baremetal.md) · 벤치마크 [바로가기](lessons/08-3-starrocks-analytics-bmt.md) · 아카이브 적합성 [바로가기](lessons/08-4-starrocks-archive-fitness.md) · 운영 명령 [바로가기](lessons/08-5-starrocks-ops.md) · 스크립트 [바로가기](scripts/08-starrocks/)
...
- [k8s 운영](lessons/ops-k8s.md) · [Ceph 운영](lessons/ops-ceph.md) · [StarRocks 운영](lessons/08-5-starrocks-ops.md) · [Ingress 운영](lessons/04-2-ingress-ops.md)
- [리눅스 기본 상식](lessons/ops-linux-basics.md) — 위 운영 문서들에 나오는 명령을 이해하기 위한 systemd/UFW/디스크/LVM/SSH 등 공통 기초

`lessons/`에 목적, 스크립트별 실행 명령, 설계 결정, 알려진 이슈가 정리되어 있다. `scripts/`에는 실행 스크립트 파일만 있다. `concepts/`에는 [Kubernetes 개념 정리](concepts/01-kubernetes.md), [Ceph 개념 정리](concepts/02-ceph.md), [StarRocks 개념 정리](concepts/03-starrocks.md), [Vault 개념 정리](concepts/04-vault.md)처럼 배경 개념 설명을 모아둔다 — 번호는 각 개념이 처음 필요해진 순서(k8s → Ceph → StarRocks → Vault)대로 매겼다.
