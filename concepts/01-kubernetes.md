...
- GPU 전용 taint는 걸지 않았다 — 왜 안 걸어도 되는지, 걸었다가 뺀 경위는 아래 "스케줄링 제어 > Taint / Toleration" 참고.
...
- **컨트롤플레인**: 클러스터 전체를 관리하는 두뇌 역할 (구성 요소는 위 "1. 정적 파드" 참고).
...
## 멀티 데이터센터로 확장한다면 (아직 실제로 하지 않음, 설계 논의만 정리)

지금은 노드 3대가 전부 같은 서브넷(10.5.5.0/24)에 있어서 드러나지 않는 문제들이다. 나중에 DC를 진짜로 나누게 되면 아래를 검토해야 한다.

### 컨트롤플레인(etcd) — DC가 정확히 2곳이면 "자동"과 "안전"을 동시에 못 가짐
- VIP(keepalived/VRRP)는 같은 서브넷(L2)에서만 동작해서 DC 사이에는 못 쓴다 — 왜 VIP 대신 도메인+헬스체크 기반 페일오버로 가야 하는지는 위 "컨트롤플레인 HA" 절 참고.
- etcd 멤버를 DC 2곳에 나누면 홀수를 어떻게 쪼개도 한쪽이 항상 더 많이 가진다(예: 2+1) — 그 쪽이 죽으면 남은 쪽은 절대 과반이 안 된다. 이건 지연시간이나 회선 안정성과 무관한 순수 쿼럼 수학 문제라, 응답시간을 아무리 줄여도 안 풀린다.
- 실무에서 쓰는 해결책 두 가지:
  - **위트니스(witness)**: 실제 워크로드 없이 투표만 하는 아주 가벼운 3번째 지점을 추가해서 사실상 3사이트 구성으로 만듦.
  - **Learner + 수동 승격**: DC2엔 투표권 없는 learner를 두고 실시간 복제만 받다가, DC1 장애 시 사람이 판단해서 `etcdctl member promote`로 승격시킴 — MySQL에서 "레플리카는 두되 자동 승격은 안 함"으로 정한 것과 같은 철학([`03-1-mysql-ha`](../lessons/03-1-mysql-ha.md) 참고).
- 둘 중 뭘 쓰든 "DC1이 진짜 죽었다"는 최종 판단은 사람이 해야 스플릿브레인을 피할 수 있다 — 완전 자동화는 안 된다.

### 애플리케이션 트래픽 — 액티브-액티브 + 여유 용량
컨트롤플레인과 별개로, 실제 사용자 트래픽은 평소 DC1/DC2가 각자 자기 용량의 40%씩만 써서 정상 서비스하다가, 한쪽이 죽으면 생존한 쪽이 80%까지 흡수하도록 설계하면 "노는 콜드 스탠바이" 없이도 이중화할 수 있다. 진입점은 VIP가 아니라 도메인(TTL 낮춤 + 헬스체크로 레코드 갱신) 기반이어야 서브넷 제약을 안 받는다.

### 사전에 반드시 준비해둬야 하는 것 (장애 나고 나서는 늦음)
- DC2 노드들이 이미 같은 CA로 발급된 인증서를 갖고 정식으로 클러스터에 join돼 있어야 한다 — etcd엔 인증서·kubeconfig 같은 로컬 파일이 안 들어있다 (위 "API 서버를 꼭 거쳐야 하나 — 우회 경로" 절의 정적 파드 매니페스트 얘기와 같은 맥락).
- nodeSelector/taint가 `llm001`처럼 특정 노드 이름이 아니라 역할 라벨(예: `role=gpu-node`) 기반이어야 DC2의 다른 이름을 가진 노드에서도 그대로 먹힌다.
- `hostPath`처럼 특정 노드의 로컬 디스크에 물린 건 DC를 못 넘는다 — DB/카프카 같은 상태 저장 계층은 k8s 밖에서 별도 복제 전략이 필요하다(이 문서의 범위 밖).

---

[다음: Ceph 개념 →](02-ceph.md)
