...
- 실무에서 쓰는 해결책 두 가지:
  ...
  - **Learner + 수동 승격**: DC2엔 투표권 없는 learner를 두고 실시간 복제만 받다가, DC1 장애 시 사람이 판단해서 `etcdctl member promote`로 승격시킴 — MySQL에서 "레플리카는 두되 자동 승격은 안 함"으로 정한 것과 같은 철학([`03-mysql-ha`](../lessons/03-mysql-ha.md) 참고).
