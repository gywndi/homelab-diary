...
AWS Secrets Manager 같은 시크릿 관리 시스템을 자체 호스팅해보면서 배운 개념을 정리한다. 배포 절차는 [lessons/11-vault.md](../lessons/11-vault.md) 참고.

## Vault는 AWS Secrets Manager의 포크가 아니다

둘은 계보상 아무 관계가 없다 — HashiCorp Vault(2015년 출시, 오픈소스로 시작)가 AWS Secrets Manager(2018년 출시, 클로즈드 소스)보다 오히려 3년 먼저 나왔다. 같은 "시크릿 관리"라는 문제를 각자 독립적으로 푼 별개 제품이다. 기능은 유사해도(암호화 저장, 세밀한 접근 제어, 로테이션, 감사 로그) **운영 모델은 근본적으로 다르다** — 이게 이 문서의 핵심 주제다.
