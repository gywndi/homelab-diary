...
설치 방법은 [설치](starrocks-install.md), 실측 벤치마크와 이번 검증에서 발견한 함정은 [BMT](starrocks-bmt.md), 장기보관 DB 적합성(압축률/리스트조회/COUNT/LIKE)은 [장기보관 DB 적합성](starrocks-archive-fitness.md), SQL 예시는 [사용쿼리 예시](starrocks-query-examples.md), 실제 동작하는 클라이언트 코드는 [어플리케이션 샘플](starrocks-app-sample.md), 프로덕션 운영 사례 기반 튜닝 포인트는 [튜닝 포인트](starrocks-tuning.md) 참고.
