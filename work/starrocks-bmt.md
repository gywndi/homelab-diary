...
지금까지는 실시간 조회/적재 성능 위주였는데, 압축률/리스트 조회(페이지네이션)/
`COUNT(*)`/`LIKE` 검색 관점에서 "MySQL 콜드 데이터를 옮겨 장기보관하는 용도"로
쓸 수 있는지는 별도 문서로 정리했다 — **[StarRocks 장기보관 DB 적합성](starrocks-archive-fitness.md)**.

한 줄 결론: 압축률(ZSTD 기준 약 6.7배)·리스트 조회·`COUNT(*)`·`LIKE` 검색 모두
실사용 가능한 수준이지만, 압축은 ZSTD로, 페이지네이션은 커서(keyset) 방식으로,
긴 텍스트 `LIKE` 전체 스캔 검색은 선택적 필터와 함께 쓰는 세 가지 설계 규칙을
지켜야 한다. 테스트 쿼리·시나리오·수치는 위 문서 참고.
...
압축률/리스트 조회/`COUNT(*)`/`LIKE` 검색 관련 백로그는 [장기보관 DB 적합성
문서](starrocks-archive-fitness.md#검증하고-싶은-것-백로그)로 옮겼다.
