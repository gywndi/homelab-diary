...
**결론**: StarRocks를 MySQL 콜드 데이터의 장기보관처로 쓰는 건 **압축률(4~7배)과
리스트 조회 성능(커서 방식 기준) 모두 실사용 가능한 수준**이다. 단 두 가지 설계
규칙을 지켜야 한다 — **(1) 압축은 ZSTD로**(쓰기 빈도가 낮은 아카이브 워크로드에
유리), **(2) 리스트/페이지네이션 API는 반드시 커서(keyset) 방식으로 설계**하고
`OFFSET` 기반 페이지 이동은 피해야 한다(깊은 페이지에서 심하게 느려짐).
