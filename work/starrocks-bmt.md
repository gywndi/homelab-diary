...
**결론**: "CN을 늘리면 무조건 좋다"가 아니다. 쿼리가 가벼우면(스캔량이 적으면) CN을 늘릴수록 오히려 손해고, 무거운 쿼리는 실제 병렬 처리 이득이 있지만 그마저도 쿼리 구조(조인 형태, dimension 크기)에 따라 특정 CN 개수에서 수렴한다. "CN 개수 = 성능"이라는 단순 공식은 성립하지 않고, 워크로드별로 적정 CN 개수를 실측으로 찾아야 한다.
...
| CN 1→2→3, 대용량 3-way JOIN(1000만~5000만 행) | CN 1→2는 **약 2배 개선**, 2→3은 개선 없음(수렴) |
...
- [x] ~~CN 개수(1/2/3)에 따른 성능 변화 확인~~ → 소규모 쿼리는 CN 늘릴수록 손해, 대용량 3-way JOIN은 1→2에서 2배 개선 후 2→3에서 수렴(1000만/5000만 행 둘 다 동일 패턴) — 위 "CN 개수 확장이 실제로 도움이 되는가" 섹션
- [ ] CN=3에서 왜 더 이상 개선이 없는지(dimension broadcast 비용 vs FE 결과 병합 비용 vs 다른 원인) `EXPLAIN`으로 실행계획 직접 확인해 분리
...
- CN 개수 확장 테스트: `ALTER SYSTEM ADD/DROP COMPUTE NODE`로 CN을 1↔2↔3 반복 등록/해제, 대용량 쿼리는 `SELECT NOW(6)`로 감싼 동일 쿼리를 5회 반복해 2~5회차 평균으로 콜드 스타트 노이즈 배제(별도 스크립트화는 아직 안 함, 인터랙티브로 진행)
